Kinetic.GameCircle = (function() {
	...
	var Class = $.Class({
		...
		_animatePress: function() {
			var animating = this.transAnim && this.transAnim.isRunning();
			var active = this.isActive();
			var actualRadius = this._calcRadius();
			var currRadius = this.getRadius();
			var expandRadius = !animating ? currRadius : active ? this._calcRadius(1) : actualRadius;
			var normalRadius = animating || active ? actualRadius : currRadius;
		...
		_removeCircle: function(pressed) {
			...
			this.transitionTo({
				...
				callback : function() {
					...
					parent.fire((pressed ? 'pressed' : 'disconnected') + 'Removed', { circle: this, neighbours: neighs });
				}.bind(this)
			});
	...
	Kinetic.Node.addGettersSetters(Class, [ 'radius', 'score' ]);
